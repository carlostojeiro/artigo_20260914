...
% [TROCA DE DATASETS] Metodos/descricoes atualizados para Edge-IIoTset + TON_IoT
% (IoT/IIoT reais), substituindo UNSW-NB15 e CICIDS2017. Os NUMEROS de
% resultados (Tabela~\ref{tab:cross}, Tabela~\ref{tab:robust}, Tabela~\ref{tab:cost},
% abstract, intro, conclusao) permanecem os da execucao anterior (com IoT-23
% valido) e serao sobrescritos pelos CSVs reais quando a re-execucao no Colab
% dos notebooks aula_edge_iiotset.ipynb e aula_ton_iot.ipynb retornar.
...
($\sim$6--8\,h) acceptable. We report the results in
Table~\ref{tab:robust} verbatim from the returned
{\tt resultados\_oof10\_<dataset>.csv} aggregation (no estimates are
inserted, and no value is invented): the Edge-IIoTset and TON\_IoT rounds
are complete, and IoT-23 remains \emph{pending} its {\tt [OOF-10]} run.

\begin{table}[!t]
\centering
\caption{Out-of-fold robustness (mean\,$\pm$\,std over 10 folds; FN is the
security-critical total). Values reported verbatim from
{\tt resultados\_oof10\_<dataset>.csv} (Edge-IIoTset and TON\_IoT
concluded; IoT-23 \emph{pending} the remaining {\tt [OOF-10]} run).}
\label{tab:robust}
\footnotesize
...
\multirow{4}{*}{IoT-23}
 & LSTM,\,Original        & \emph{pending} & \emph{pending} & \emph{pending} & \multicolumn{2}{c}{\emph{pending}}\\
 & LSTM,\,SMOTETomek      & \emph{pending} & \emph{pending} & \emph{pending} & \multicolumn{2}{c}{\emph{pending}}\\
 & LSTM,\,WGAN-GP         & \emph{pending} & \emph{pending} & \emph{pending} & \multicolumn{2}{c}{\emph{pending}}\\
 & LSTM,\,cWGAN-GP        & \emph{pending} & \emph{pending} & \emph{pending} & \multicolumn{2}{c}{\emph{pending}}\\
\bottomrule
\end{tabular}
\end{table}
