...
\markboth{IEEE Access,~Vol.~XX, No.~XX, Month~Year}%
{Tojeiro \MakeLowercase{\textit{et al.}}: Reducing False Negatives in IoT Malware Detection}

\maketitle

\begin{abstract}
Class imbalance is one of the most harmful properties of real-world network traffic for machine-learning-based intrusion detection systems (IDSs): when attacks are rare relative to benign traffic, detectors become biased toward the majority class and silently miss malware---a false negative (FN) that can translate directly into a compromised device. Although generative adversarial networks (GANs) are increasingly used to oversample minority attack classes, most studies report aggregate accuracy or F1-score, rarely quantifying the impact of the balancing strategy on the false negative count (the error of highest cost in security), and almost never isolate the balancing effect from the classifier choice. This paper presents a strictly controlled comparison of six data-balancing strategies---(i) no balancing, (ii) SMOTETomek, (iii) a vanilla GAN, (iv) WGAN-GP, (v) conditional WGAN-GP (cWGAN-GP), and (vi) the conditional tabular GAN (CTGAN)---on an identical protocol reproduced on three real public datasets (CICIDS2017, UNSW-NB15, and the IoT-23 traffic collected by the Stratosphere Laboratory) and four classifiers (MLP, XGBoost, Random Forest, and LSTM). All strategies share the same workload (40{,}000 flows, 82\% benign / 18\% attack), split, feature-selection step, and test set, so that any difference is attributable to the balancing method alone. The results show that the benefit is strongly classifier-dependent: the deep sequential model (LSTM) is the most degraded by imbalance and the most improved by balancing, with FNs cut by 83.1\% on CICIDS2017 (1{,}716$\rightarrow$290), 81.5\% on UNSW-NB15 (845$\rightarrow$156), and 40.6\% on IoT-23 (1{,}169$\rightarrow$694); tree ensembles, already near-saturated under imbalance, gain only marginally; and the MLP gains are inconsistent, even regressing on easy problems. No single strategy dominates: classic SMOTETomek and Wasserstein-based generative methods alternate as best depending on model and dataset. These nuances are masked by aggregate metrics, motivating a confusion-matrix-oriented reporting standard for imbalance studies in NIDS.
\end{abstract}
...
These nuances are invisible in accuracy- or F1-only reporting, which motivates our main methodological recommendation: imbalance studies in NIDS should report confusion-matrix counts and isolate the balancing effect from the classifier choice on multiple real datasets. Our protocol provides a faithful, reproducible template for doing so.
