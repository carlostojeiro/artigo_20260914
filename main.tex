...
Table~\ref{tab:robust} as \emph{pending}: the runs were launched but the
numbers will be reported verbatim from the returned
{\tt resultados\_oof10\_<dataset>.csv} aggregation (no estimates are
inserted, and no value is invented).

\begin{table}[!t]
\centering
\caption{Out-of-fold robustness (mean\,$\pm$\,std over 10 folds; FN is the
security-critical total). \emph{Pending} the completion of the
{\tt [OOF-10]} runs on the Colab notebooks ($\sim$6--8\,h).}
\label{tab:robust}
\footnotesize
