...
\subsection{Cost-Effectiveness: False-Negative Reduction versus Training Time}
\label{sec:fntt}

Because the LSTM is the classifier where balancing matters most, Table~\ref{tab:fntt} confronts, per dataset, the false-negative (FN) count of the LSTM in each scenario with the wall-clock time required to produce it (generator training time; SMOTETomek and the no-balancing baseline add negligible overhead). Three patterns hold across all datasets. First, SMOTETomek offers the best cost--benefit: it cuts FNs in seconds ($-$89.9\% on TON\_IoT, $-$41.2\% on IoT-23, $-$12.2\% on Edge-IIoTset) at zero generator cost. Second, among generative methods WGAN-GP is the most efficient, recovering $-$75.5\% (TON\_IoT), $-$41.3\% (IoT-23), and $-$14.1\% (Edge-IIoTset) of the attacks in about 3.5 minutes, closely followed by CTGAN on TON\_IoT ($-$89.1\% in 4.7 minutes). Third, the vanilla GAN achieves the lowest FN on Edge-IIoTset and IoT-23 (716 and 554; reductions of $-$38.1\% and $-$53.4\%) but requires 16--21~minutes---the most expensive generator---and cWGAN-GP is the least cost-effective, even \emph{increasing} FNs on Edge-IIoTset ($1{,}156\rightarrow1{,}445$). In practice, a training-time-constrained operator should start from SMOTETomek; if generative oversampling is required, WGAN-GP offers the best FN-per-minute trade-off, reserving the vanilla GAN for the hardest cases where its extra cost is repaid by the lowest FN count. Fig.~\ref{fig:fntt} visualizes these trade-offs.

\begin{table}[!t]
\centering
\caption{LSTM cost--effectiveness per dataset: false negatives (FN on the fixed 12{,}000-flow test set) versus generator training time (TT, in seconds) per balancing scenario. $\Delta$FN is relative to the unbalanced baseline.}
\label{tab:fntt}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lrcrr@{}}
\toprule
Dataset & Scenario & FN & TT (s) & $\Delta$FN \\
\midrule
\multirow{6}{*}{Edge-IIoTset}
 & Original        & 1{,}156  & 0 & --- \\
 & SMOTETomek      & 1{,}015  & $\approx$0 & $-12.2\%$ \\
 & GAN             & 716      & 971 & $-38.1\%$ \\
 & WGAN-GP         & 993      & 222 & $-14.1\%$ \\
 & cWGAN-GP        & 1{,}445  & 273 & $+25.0\%$ \\
 & CTGAN           & 1{,}039  & 321 & $-10.1\%$ \\
\multirow{6}{*}{TON\_IoT}
 & Original        & 1{,}930  & 0 & --- \\
 & SMOTETomek      & 194      & $\approx$0 & $-89.9\%$ \\
 & GAN             & 253      & 1{,}280 & $-86.9\%$ \\
 & WGAN-GP         & 472      & 207 & $-75.5\%$ \\
 & cWGAN-GP        & 1{,}611  & 253 & $-16.5\%$ \\
 & CTGAN           & 211      & 282 & $-89.1\%$ \\
\multirow{6}{*}{IoT-23}
 & Original        & 1{,}190  & 0 & --- \\
 & SMOTETomek      & 700      & $\approx$0 & $-41.2\%$ \\
 & GAN             & 554      & 953 & $-53.4\%$ \\
 & WGAN-GP         & 698      & 211 & $-41.3\%$ \\
 & cWGAN-GP        & 702      & 265 & $-41.0\%$ \\
 & CTGAN           & 833      & 130 & $-30.0\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/fig5_fntt.pdf}
\caption{LSTM cost--effectiveness: false negatives on the fixed test set (vertical axis) versus generator training time in seconds (horizontal axis) for each balancing scenario and dataset. SMOTETomek (TT$\approx$0) occupies the top-left effective frontier, followed by the cheap WGAN-GP generator.}
\label{fig:fntt}
\end{figure}
